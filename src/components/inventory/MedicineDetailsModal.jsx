    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="medicine-details-title">
      ...
      <div className="relative w-full max-w-4xl max-h-[calc(100vh-2rem)] overflow-y-auto bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/70 dark:border-slate-800 shadow-2xl">
        ...
        <div className="p-5 sm:p-6 space-y-6">
          ...
          <section className="pt-5 border-t border-slate-100 dark:border-slate-800"><h4 className="text-sm font-bold text-slate-900 dark:text-white mb-4">Batch Information</h4><dl className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"><Detail label="Batch Number">{medicine.batchNumber}</Detail><Detail label="Manufacturing Date">{formatDate(medicine.manufacturingDate)}</Detail><Detail label="Expiry Date">{formatDate(medicine.expiryDate)}</Detail><Detail label="Barcode">{medicine.barcode}</Detail></dl></section>
          <section className="pt-5 border-t border-slate-100 dark:border-slate-800"><div className="flex items-center justify-between gap-3"><h4 className="text-sm font-bold text-slate-900 dark:text-white">Purchase Batches</h4><button type="button" onClick={() => setShowBatches((value) => !value)} className="px-3 py-2 text-xs font-semibold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/50 rounded-lg">{showBatches ? 'Hide Batches' : 'View Batches'}</button></div>{showBatches && <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[750px] text-xs"><thead><tr className="border-b border-slate-100 dark:border-slate-800"><th className="text-left pb-2">Batch</th><th className="text-left pb-2">Supplier</th><th className="text-left pb-2">Location</th><th className="text-left pb-2">Expiry</th><th className="text-left pb-2">Purchase Price</th><th className="text-left pb-2">Selling Price</th><th className="text-left pb-2">Received</th><th className="text-left pb-2">Remaining</th><th className="text-left pb-2">Status</th></tr></thead><tbody>{(medicine.batches || []).map((batch) => <tr key={batch.id} className="border-b border-slate-50 dark:border-slate-800/40"><td className="py-2">{batch.batchNumber}</td><td className="py-2">{batch.supplierName}</td><td className="py-2">{batch.rackLocation || medicine.rackLocation || '—'}</td><td className="py-2">{batch.expiryDate}</td><td className="py-2">{batch.purchasePrice}</td><td className="py-2">{batch.sellingPrice}</td><td className="py-2">{batch.quantityReceived}</td><td className="py-2">{batch.quantityRemaining}</td><td className="py-2">{batch.status}</td></tr>)}</tbody></table></div>}</section>
