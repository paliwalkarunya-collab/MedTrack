...
export const CustomerDetailsCard = ({ billNumber, customer, onChange, paymentMethod, onPaymentMethodChange }) => (
  <section className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200/70 dark:border-slate-800/70 shadow-xs">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <label className="sm:col-span-2"><span className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Payment Method</span><select value={paymentMethod} onChange={(event) => onPaymentMethodChange(event.target.value)} className={inputClassName}><option>Cash</option><option>UPI</option><option>Card</option><option>Other</option></select></label>
    </div>
  </section>
);
