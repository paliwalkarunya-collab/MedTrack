...
import { LogOut, Menu, Search, Bell, Moon, Sun, User, HeartPulse } from 'lucide-react';
...
export const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-20 h-[76px] bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-[#d9e9ec] dark:border-slate-800/70 px-4 lg:px-8 flex items-center justify-between gap-4 transition-colors shadow-[0_2px_12px_rgba(17,58,79,0.03)]">
      ...
      <div className="flex items-center gap-3.5">
        ...
        <div className="hidden xl:flex h-9 w-9 items-center justify-center rounded-xl bg-[#e6f7f8] text-[#079bb0]"><HeartPulse className="h-5 w-5" /></div>
        <div>
        <p className="hidden lg:block text-[10px] font-bold uppercase tracking-[0.16em] text-[#079bb0]">MedTrack Clinic</p>
        <h1 className="text-xl font-bold text-[#11364f] dark:text-white tracking-tight hidden sm:block">
          {title}
        </h1>
        </div>
      </div>
      ...
      <div className="flex-1 max-w-xl mx-2 sm:mx-6">
        <div className="relative group">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-slate-500 group-focus-within:text-[#079bb0] transition-colors" />
          <input
            type="text"
            placeholder="Search medicines, categories, suppliers..."
            className="w-full pl-10 pr-20 py-2.5 text-sm bg-[#f4f9fa] dark:bg-slate-800/60 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 rounded-xl border border-[#dcebed] dark:border-slate-700/60 focus:border-[#079bb0] focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-4 focus:ring-[#079bb0]/10 shadow-search transition-all duration-200"
          />
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        >
          ...
          <span className="absolute top-2 right-2 w-2 h-2 bg-[#079bb0] rounded-full ring-2 ring-white dark:ring-slate-900" />
        </button>

        {/* User Profile */}
        <div className="flex items-center gap-3 pl-3 border-l border-slate-200/70 dark:border-slate-800/70">
          <div className="w-9 h-9 rounded-xl bg-[#e6f7f8] dark:bg-blue-950/80 text-[#079bb0] dark:text-blue-400 flex items-center justify-center font-semibold text-sm ring-1 ring-[#079bb0]/20 shadow-2xs">
